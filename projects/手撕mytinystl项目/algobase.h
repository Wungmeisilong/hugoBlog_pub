...
#include "iterator.h"  // 迭代器头文件
#include "util.h"       //move, forward, swap 等函数

namespace mystl
{

#ifdef max
#pragma message("#undefing marco max") // 预处理器指令，检查是否已经定义了宏 max
#undef max
#endif // max

#ifdef min
#pragma message("#undefing marco min")  // 预处理器指令，检查是否已经定义了宏 min
#undef min
#endif // min
...
unchecked_copy_cat(InputIter first, InputIter last, OutputIter result, 
                   mystl::input_iterator_tag) // 编译器期多态，只有输入迭代器才能调用这个版本
{
...
// 针对指针类型的 unchecked_copy 特化版本
// 仅当 Tp 去 const 后与 Up 相同，且 Up 支持平凡赋值时才启用（SFINAE）
// 这样可以直接用 memmove 进行高效内存拷贝
template <class Tp, class Up>
typename std::enable_if<
  std::is_same<typename std::remove_const<Tp>::type, Up>::value && // Tp 去 const 后和 Up 相同
  std::is_trivially_copy_assignable<Up>::value,                    // Up 是平凡赋值类型，保证类型可以用内存块拷贝，不需要逐元素赋值。
  Up*>::type                                                       // 返回类型为 Up*
unchecked_copy(Tp* first, Tp* last, Up* result)
{
  const auto n = static_cast<size_t>(last - first); // 计算元素个数
  if (n != 0)
    std::memmove(result, first, n * sizeof(Up));    // 保证类型可以用内存块拷贝，不需要逐元素赋值。
  return result + n;                                // 返回拷贝后新位置
}
...
//对于自定义类或复杂类型（非平凡赋值），则不会用这个版本，而是用普通的逐元素拷贝。
template <class Tp, class Up>
typename std::enable_if<
...
//一元操作：只接受一个参数的函数或函数对象，在这里通常是一个判断条件，比如判断元素是否为偶数、是否大于0等。
/*****************************************************************************************/
template <class InputIter, class OutputIter, class UnaryPredicate>
OutputIter 
copy_if(InputIter first, InputIter last, OutputIter result, UnaryPredicate unary_pred)
{
  for (; first != last; ++first)           // 遍历输入区间
  {
    if (unary_pred(*first))                // 如果当前元素满足一元操作（谓词）
      *result++ = *first;                  // 拷贝到输出区间，并移动输出迭代器
  }
  return result;                           // 返回输出区间的新尾后迭代器
}
...
// 为 one-byte 类型（如 char、unsigned char、int8_t 等）提供特化版本
// 该版本利用 std::memset 实现高效批量填充
template <class Tp, class Size, class Up>
typename std::enable_if<
  std::is_integral<Tp>::value && sizeof(Tp) == 1 &&         // Tp 是整数类型且占1字节
  !std::is_same<Tp, bool>::value &&                         // Tp 不是 bool
  std::is_integral<Up>::value && sizeof(Up) == 1,           // Up 也是1字节整数类型
  Tp*>::type                                                // 返回 Tp* 类型
unchecked_fill_n(Tp* first, Size n, Up value)
{
  if (n > 0)
  {
    // 用 memset 批量填充内存，效率远高于逐元素赋值
    std::memset(first, (unsigned char)value, (size_t)(n));
  }
  return first + n; // 返回填充后区间的尾后指针
}
...
void fill_cat(RandomIter first, RandomIter last, const T& value,
              mystl::random_access_iterator_tag)
{
  fill_n(first, last - first, value);//last - first类型为size_t
}
                             ...
                             const unsigned char* last2)
{
  ...
  // 先比较相同长度的部分
  const auto result = std::memcmp(first1, first2, mystl::min(len1, len2));//std::memcmp用于按字节比较两个内存块的内容，比较的方法是mystl::min(len1, len2),返回结果为
  // 若相等，长度较长的比较大
